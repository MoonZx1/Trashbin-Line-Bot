...
void handleTrashBin(int binId, float weightKg, bool &hasNotifiedCloseToFull, bool &hasNotifiedFull, bool &hasNotifiedEmpty, int relayX) {
  ...
  // กรณีถังใกล้เต็ม
  if (weightKg >= 2.00 && weightKg < 4.00 && !hasNotifiedCloseToFull) {
    LINE.notify("ถังขยะ " + String(binId) + " ใกล้เต็มแล้ว (น้ำหนัก: " + String(weightKg, 2) + " กิโลกรัม)");
  ...
  // กรณีถังเต็ม
    ...
    LINE.notify("ถังขยะ " + String(binId) + " เต็มแล้ว กรุณานำไปเท (น้ำหนัก: " + String(weightKg, 2) + " กิโลกรัม)");
  ...
  // กรณีถังว่างหลังเท
    ...
    LINE.notify("ถังขยะ " + String(binId) + " ถูกนำไปเทแล้ว ระบบพร้อมใช้งานใหม่");
